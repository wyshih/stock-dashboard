...
def _half_year(ts: pd.Timestamp) -> str:
...
def page_fpm_rules() -> None:
    ...
    st.caption("另一個獨立專案（fpm）從歷史資料挖出的「平盤起漲點」規則，"
               "以下只顯示**2025-02 之後**的歷史命中買點（比全站其他頁的測試期範圍新，"
               "fpm 資料持續累積中，見下方「驗證等級」說明）——"
    ...
    selected_levels = st.multiselect(
        ...
        help="「window顯著性驗證過」：規則在該筆所屬的 walk-forward 視窗有通過統計檢定。"
             ...
             "「待驗證」：最近 20 個交易日內的命中，20 天後的結果還沒算出來，"
             "純粹是「現在符合型態」，過陣子資料更新後才會知道結果。",
